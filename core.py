...
def run_nn_refac01(data_name,data_set,data_end_index,fea_dict,lab_dict,arch_dict,cfg_file,processed_first,next_config_file):
    ...
    def _optimization_step(optimizers, outs_dict, config, arch_dict):
        ...
        #M = list(nns["CNN_layers"].parameters())[2]
        #r1 = torch.norm(M,p=1)
        outs_dict['loss_final'].backward()
        #outs_dict['loss_final']=outs_dict['loss_final'] + LAMBDA*r1
...
def run_nn(data_name,data_set,data_end_index,fea_dict,lab_dict,arch_dict,cfg_file,processed_first,next_config_file):
    ...
    # Count and print the number of trainanble parameters
    #number_of_train_param = 0
    #for net in nns.keys():
    #    number_of_train_param += sum(p.numel() for p in nns[net].parameters() if p.requires_grad)

    #print("Number of trainable parameters:")
    #print(number_of_train_param)
    ...
    with open(info_file, "w") as text_file:
        ...
        if to_do!='forward':
            ...
            text_file.write("err=%s\n" % err_tot.cpu().numpy())
        text_file.write("elapsed_time_chunk=%f\n" % elapsed_time_chunk)
        freqs = list(nns["CNN_layers"].parameters())[0]
        bands = list(nns["CNN_layers"].parameters())[1]
        for ii in range(len(freqs)):
            low = 50 + 16000*abs(freqs[ii].item())
            high = low + 50 + 16000*abs(bands[ii].item())
            text_file.write("fc1_"+str(ii)+"=%f, " % low)
            text_file.write("fc2_"+str(ii)+"=%f\n" % high)
